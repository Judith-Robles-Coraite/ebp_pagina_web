<template>
    <div class="relative">
      <div class="bg-white py-2 px-4 md:px-8 lg:px-20 mx-auto">
        <h2 class="text-center text-2xl md:text-3xl font-bold mb-6 md:mb-0 text-[#0A8B37]">
          ESPECIALIDADES EN OFERTA
        </h2>
      </div>
  
      <!-- Botón Izquierdo -->
      <button 
        class="absolute left-20 top-1/2 transform -translate-y-1/2 bg-[#f1efef] text-[#0A8B37] px-4 py-4 z-50 hover:bg-opacity-50 rounded-full"
      ...
      <!-- Imagen del programa -->
      <div class="w-1/4 flex justify-center items-center ml-14"> <!-- Cambiado a w-1/4 -->
      ...
      <div class="w-3/4 bg-white border border-[#0A8B37] rounded-3xl px-0 shadow-lg  ml-4 mr-20 "> <!-- Cambiado a w-3/4 y añadido ml-4 -->
        <h2 class="bg-gradient-to-r from-[#1c4d59] to-[#0A8B37] text-[#ffffff] p-6 rounded-3xl rounded-b-none text-2xl font-bold text-center ">{{ currentProgram.title }}</h2>
        <p class="mt-2 text-[#0A8B37] font-bold text-center ">{{ currentProgram.description }}</p>
        ...
        <!-- Botón Derecho -->
        <button 
          class="absolute right-20 top-1/2 transform -translate-y-1/2 bg-[#f1efef] text-[#0A8B37] px-4 py-4 z-50 hover:bg-opacity-50 rounded-full"
          @click="setCurrentProgram((currentProgramIndex + 1) % programs.length)">
          <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#0A8B37">
      ...
      <div class="flex justify-center space-x-2 mt-2 ">
        <button 
          ...
          :class="{'bg-[#0A8B37] text-white': currentProgramIndex === index, 'bg-gray-200': currentProgramIndex !== index}"
    ...
    <footer class="bg-gradient-to-r from-[#1c4d59] to-[#0A8B37] text-white py-3 pb-2 px-4 mt-4">
    <div class="flex justify-center space-x-4">
      <div class="inline-flex rounded-full border border-[#ffffff] p-1">
      <!-- Botón de Maestrías en Desarrollo -->
      <button
        @click="changeShowing('desarrollo')"
        :class="{
          'bg-white text-[#0A8B37]': showing === 'desarrollo',
          'text-white hover:bg-[#d7d7d7ae] hover:text-[#ffffff]': showing !== 'desarrollo'
        }"
      ...
      <!-- Botón de Maestrías Concluidas -->
      <button
        @click="changeShowing('concluidas')"
        :class="{
          'bg-white text-[#0A8B37]': showing === 'concluidas',
          'text-white hover:bg-[#d7d7d7ae] hover:text-[#ffffff]': showing !== 'concluidas'
        }"
        class="rounded-full px-2 py-1 transition-colors duration-300"
      >
  ...
  export default {
    data() {
      return {
        ...
        programs: [
          {
            title: "Especialidad en Hematología y Hemoterapia",
          ...
          {
            title: "Especialidad en Ultrasonografia",
            shortTitle: "Hematología",
    ...
    methods: {
      ...
      autoSlide() {
        this.autoSlideInterval = setInterval(() => {
          this.currentProgramIndex = (this.currentProgramIndex + 1) % this.programs.length;
          this.currentImage = this.programs[this.currentProgramIndex].image1;
