  ...
  <div class="bg-white px-5 md:px-8 lg:px-10 py-2 lg:py-4 mx-auto">
    <div class="w-full text-center mb-0">
      ...
      <p class="text-gray-700 mt-2 sm:mt-4 px-0 sm:px-12 md:px-36">
        Contamos con múltiples sedes a nivel nacional, estratégicamente ubicadas
        para ofrecerte acceso a una educación de calidad sin importar dónde te
        encuentres. Conoce nuestras ubicaciones y elige la que mejor se adapte a
        tus necesidades.
      </p>
    </div>
    <div
    ...
    >
      ...
      >
        <!-- Navbar de Sedes -->
        <div
          class="sedes-navbar flex justify-center border-b border-[#0A8B37] mb-2"
        >
          <a
            v-for="sede in sedes"
            :key="sede.nombre"
            ...
            @click="
              () => {
                ...
                //si la sede seleccionada no tiene la opcion lab se seleccina location si resulta  que teine no se hace nada
                showing === 'lab' &&
                !sede.botones.some((boton) => boton.tipo === 'lab')
        ...
        <div v-for="sede in sedes" :key="sede.nombre">
          <h2
            ...
            class="text-2xl font-bold text-center text-[#0A8B37] mb-2"
          >
            {{ sede.titulo }}
          </h2>
        </div>
          ...
          <div v-if="selectedSede === sede.nombre" class="sede-info">
            ...
            <div v-if="showing === 'location'">
              ...
              <Sede
                ...
                :socialMedia="sede.location.socialMedia"
              />
            </div>
            <div v-if="showing === 'lab'">
              <h2>Lab en desarrollo</h2>
            </div>
          </div>
        </div>
...
const sedes = ref([
  ...
  },
  {
    nombre: "oruro",
    titulo: "Oruro",
    galeria: "Galería de Oruro",
    location: {
      direction:
        "Calle Junín, (entre Pagador y Potosí) Edif. Divina Providencia 1er piso Of. 18",
      mapLocation:
