<template>
  <div class="">
    <svg
      class="w-full h-full"
      viewBox="280 0 1600 1770"
      xmlns="http://www.w3.org/2000/svg"
    >
      <defs>
        <linearGradient id="mapGradient" x1="0%" y1="50%" x2="100%" y2="50%">
          <stop offset="0%" style="stop-color: #203c61; stop-opacity: 1" />
          <stop offset="70%" style="stop-color: #0a8b37; stop-opacity: 1" />
        </linearGradient>
      </defs>
      <!-- SANTACRUZ  -->

      <path
        transform="translate(1345,479)"
        d="m0 0h13l10 2 12 5 22 10 23 11 11 8 6 7 3 7 1 14-1 10v14l4 12 8 15 1 4v13l-6 16-3 11v10l1 6v18l-3 15 1 10 7 14 10 15 4 9 3 15 4 40 4 45 7 8 9 3 39 1 56 2h29l22-1 40-4 16-3 13-4h14l5 3 1 2v10l-6 18-8 20-4 16-2 18v18l2 15 5 12 10 9 14 10 10 7 6 3 17 3 6 4 1 2v7l-3 15 1 9 5 12 14 18 8 16 4 16 1 8v19l-3 20-2 9-1 19v14l-5 10-8 10-13 17-12 19-14 25-4 10-1 4v11l4 6 9 7 6 8 4 8 1 5-4 3-17 4-13 2-3-8-5-19-6-10-10-8-14-8-16-6-8-2-13-5-32-16-37-16-15-4-14-2h-43l-23 4-20 6-34 11-27 6-57 9-40 8-15 5-11 7-4 2-2 4-7 9-10 21-8 19-6 11-13 16-8 14-5 11-104 1-9 1h-15l-7-3-11-11-6-1-7 6-4 5-4 2-7-1-2-2-2-14-3-43-2-34-1-27-1-50-1-3-7-2h-21v16l-4 1-6-7-5-12-8-27-6-9-16-8-8-6-1-3h-2l-1-4 7-12 2-7-1-10-4-10-8-11-7-7-8-7-11-14-4-7-1-4v-12l4-5 14-8 17-8 10-9 4-10v-19l-5-12-6-8-8-8-12-15-6-4-3-1-16-2-5-5-4-6-4-13-2-11v-19l4-18 9-23 3-12v-12l-9-27v-5l17-3 40-7 27-4 34-7 46-8 14-4 6-3 2-2-1-8-6-10-10-12-5-19-4-14-4-5-3-1-11-1-3-3-3-7-2-13-9-10-6-7v-7l10-26 14-30 18-7 30-11 30-12 37-15 38-15 34-12 37-13 28-12 17-9 4-3 3-1z"
        class="hover:fill-[#24FD97]"
        @click="emitSede('santacruz')"
        :fill="esSedeActualSeleccionada('santacruz')"
      />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        x="1100"
        y="1000"
        height="80px"
        viewBox="0 -960 960 960"
        width="80px"
        class="icon-ubicacion"
        @click="emitSede('santacruz')"
        :fill="esSedeActualSeleccionadaIcon('santacruz')"
      >
        <path
          d="M480-480q33 0 56.5-23.5T560-560q0-33-23.5-56.5T480-640q-33 0-56.5 23.5T400-560q0 33 23.5 56.5T480-480Zm0 400Q319-217 239.5-334.5T160-552q0-150 96.5-239T480-880q127 0 223.5 89T800-552q0 100-79.5 217.5T480-80Z"
        />
      </svg>

      <!-- BENI -->
      <path
        transform="translate(794,96)"
        d="m0 0 3 1-1 4v9l4 10 7 10 2 9v24l-5 40-1 7v10l4 11 7 11 9 11 8 6 13 7 4 7 4 13 5 6 12 9 12 6 18 8 5 4 10 19 7 7 8 4 18 6 20 9 10 3 17 2 9-2 11-7h7l19 5 19 6 11 1 21-4 6 4 9 11 5 5 9 8 15 8 15 5 30 7 21 6 21 9 12 8 11 9 12 12 10 7 14 7 17 4 15 2v3l-15 7-27 11-19 7-30 10-21 8-32 13-46 19-39 15-30 11-3 2-11 23-10 23-6 17 1 6 7 9 7 7 4 16 3 7 1 3 5 2h11l4 4 3 13 3 15 3 8 9 10 6 9-4 4-25 6-50 9-25 5-45 7-26 4h-24l-19-3-28-5h-11l-9 3-9 7-6 7-11 26-5 15-5 5-23 12-6 3h-11l-32-5-12-5-8-7-7-10-8-12-6-7-8-5-16-5-16-6-15-8-4-8-10-21-11-17-7-11-6-12-5-18-4-17-4-10-10-15-12-12-8-7 2-5 2-4v-21l7-15 2-11 2-18 10-21 1-3v-8l-3-9 7-24 7-17 4-6 10-8 3-6 3-22 7-11 5-8 3-8 3-13 5-12 3-13 3-19 1-6 1-17 7-22v-8l-5-12-4-7-1-5 2-5 5-4 14-5 5-5 2-3v-7l-6-13 1-5 4-2 39-5 8-3 4-5 1-11v-10l4-11 7-8 4-8-1-19 2-7 3-2h19l8-4 10-9 5-7 4-12 4-6 16-10z"
        class="hover:fill-[#24FD97]"
        @click="emitSede('trinidad')"
        :fill="esSedeActualSeleccionada('trinidad')"
      />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        x="850"
        y="600"
        height="80px"
        viewBox="0 -960 960 960"
        width="80px"
        class="icon-ubicacion"
        @click="emitSede('trinidad')"
        :fill="esSedeActualSeleccionadaIcon('trinidad')"
      >
        <path
          d="M480-480q33 0 56.5-23.5T560-560q0-33-23.5-56.5T480-640q-33 0-56.5 23.5T400-560q0 33 23.5 56.5T480-480Zm0 400Q319-217 239.5-334.5T160-552q0-150 96.5-239T480-880q127 0 223.5 89T800-552q0 100-79.5 217.5T480-80Z"
        />
      </svg>

      <!-- LA PAZ -->
      <path
        transform="translate(484,301)"
        d="m0 0 6 1 22 12 42 21 23 13 22 11 13 8 4 3-1 10-5 17-6 17-5 13-6 8-4 8-2 16-2 10-5 4-6 5-8 17-6 18-4 14 2 9 1 3v8l-8 16-3 7-2 23-8 18v21l-4 9 1 6 5 6 8 7 7 7 9 14 5 15 5 24 5 14 8 14 11 15 10 18 3 6-1 7-5 9v7l6 16 1 25 4 16 8 17 9 14 9 12 5 14v9l-4 7-7 7-6 5-5 7-1 3v10l3 14 10 20v13l-1 8h-17l-19-1h-16l-10 4-9 6-12 11-10 9-14 10-11 4-14-1-16-4-11-3-10 1-17 5-18 8-44 22-10 6h-5l-6-4-10-4-5-6-6-18-9-16-5-9-4-17-5-9-6-9-1-3v-13l4-14 6-9 15-20 8-10 9-10 12-9 10-7 6-8 8-16 4-7 11-8 6-8-1-7-6-7-12-8v-4l8-9v-7l-5-5-8-3-19-3-10-9-11-11-8-7-5-8 4-16 3-11v-12l-4-11-13-21-3-11 1-13 5-14 7-12 10-13 12-14 13-17 6-13 2-8v-12l-7-20 1-9v-10l-4-9-4-6v-9l5-22 1-11 1-51 4-29 4-9 7-7 15-10 8-9 3-11 2-1 6 1h8l4-2 2-6 2-12 4-6 4-4 6-14 5-5 10 1 7-2z"
        class="hover:fill-[#24FD97]"
        @click="emitSede('lapaz')"
        :fill="esSedeActualSeleccionada('lapaz')"
      />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        x="420"
        y="850"
        height="80px"
        viewBox="0 -960 960 960"
        width="80px"
        class="icon-ubicacion"
        @click="emitSede('lapaz')"
        :fill="esSedeActualSeleccionadaIcon('lapaz')"
      >
        <path
          d="M480-480q33 0 56.5-23.5T560-560q0-33-23.5-56.5T480-640q-33 0-56.5 23.5T400-560q0 33 23.5 56.5T480-480Zm0 400Q319-217 239.5-334.5T160-552q0-150 96.5-239T480-880q127 0 223.5 89T800-552q0 100-79.5 217.5T480-80Z"
        />
      </svg>

      <!-- PANDO -->
      <path
        transform="translate(799,1)"
        d="m0 0 7 1 6 4 1 1v12l-2 16-1 28-4 13-5 10-19 10-11 7-4 2-2 4-8 19-9 9-6 4-19 1-7 3-3 6v24l-7 8h-2l-2 5-4 8-1 4-1 21-2 2-19 3-23 3-8 4-3 4v9l5 10v6l-8 4-11 4-6 5-3 6v8l4 9 6 11v8l-7 19-1 17-1 7h-3l-44-22-22-12-25-12-19-10-14-8-5-2-6 1-5 5-5 6-10-1-8 3-5 6-4 10-3 5-7 8-4 16-1 2-6-1-4-1h-5l-13-11-9-11-13-18-14-19-12-17-16-23-13-19-10-14-14-20-12-19v-2l13-2h10l47 6 11 1h13l12-2 16-6 11-8 7-7 11-14 12-13 6-5 8-3 7-1 20-1 12-6 8-9 11-19 7-7 8-3 18-4 9-5 15-12 13-9 10-7 16-7 17-5 25-5 25-9 17-6 13-3 8-1h9l19 3 10 1h14l11-4z"
        class="hover:fill-[#24FD97]"
        @click="emitSede('pando')"
        :fill="esSedeActualSeleccionada('pando')"
      />

      <!-- COCHABAMBA -->
      <path
        transform="translate(835,791)"
        d="m0 0 15 1 30 6 22 3 2 7 5 16 3 11-1 12-5 17-7 19-2 10v25l3 15 5 13 6 8 6 4 20 3 6 5 7 11 13 13 5 10 1 5v10l-4 11-8 7-18 8-11 6-7 6-2 4-1 13 2 9 6 10 11 13 16 15 8 11 3 7v9l-6 10-4 5-4 1-11-7-15-10-8-3h-10l-4 2-9 12-8 4h-11l-16-6-10-7-7-7-7-8-12-11-7-7-9-12-10-10-15-10-11-5-7-2-13-1-13 2-12 5-13 9-10 2-21-5-9-3-4-13-9-17-10-15-5-7v-25l-3-8-6-10-3-9-1-6v-8l4-6 7-7 7-6 5-10v-9l-4-15-7-12-7-9-9-16-5-11-3-15-1-22-3-11-3-9 4-6 6 2 17 8 25 7 10 7 8 11 10 16 8 7 13 6 26 5 7 1h14l16-8 19-10 4-5 6-16 10-26 6-7 10-6z"
        class="hover:fill-[#24FD97]"
        @click="emitSede('cochabamba')"
        :fill="esSedeActualSeleccionada('cochabamba')"
      />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        x="700"
        y="950"
        height="80px"
        viewBox="0 -960 960 960"
        width="80px"
        class="icon-ubicacion"
        @click="emitSede('cochabamba')"
        :fill="esSedeActualSeleccionadaIcon('cochabamba')"
      >
        <path
          d="M480-480q33 0 56.5-23.5T560-560q0-33-23.5-56.5T480-640q-33 0-56.5 23.5T400-560q0 33 23.5 56.5T480-480Zm0 400Q319-217 239.5-334.5T160-552q0-150 96.5-239T480-880q127 0 223.5 89T800-552q0 100-79.5 217.5T480-80Z"
        />
      </svg>

      <!-- SUCRE -->
      <path
        transform="translate(806,1141)"
        d="m0 0 4 1 5 4 5 2h7l5-4 1-2 4 2 7 7 11 7 11 5 7 2h11l8-3 9-8 7-8 5-1 14 7 17 12 4 2 10 2 9 10 13 6 6 4 4 5 12 36 6 9 4 3 7 1 4-3 1-14 4-1 12 2 3 2 1 68 3 66 3 29 1 7 5 2 2 1h10l5-3 8-10 4 2 10 11 7 3 5 1h12l9-1h104v54l-2 5-17 1-90 2-85 4h-16l-7-11-3-2h-5l-5 10-4 16-4 3-3 1h-20l-8-3-8-7-7-8-5-3-4-1h-7l-20 9h-8l-12-7-7-1-8 6-11-3-7-1v21l8 32-2 9h-4l-12-13-8-10-3-8v-33l10-74v-17l-4-13 1-5 5-4 12-5 18-6 10-6 28-22 4-6v-7l-6-12-10-11-5-13-4-4-11-5-3-1h-8l-7 5h-10l-8-4-10-10-11-15-5-12v-15l3-13v-7l-2-4-4-1-10 2-1-5 2-10v-7l-7-15z"
        class="hover:fill-[#24FD97]"
        @click="emitSede('sucre')"
        :fill="esSedeActualSeleccionada('sucre')"
      />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        x="820"
        y="1170"
        height="80px"
        viewBox="0 -960 960 960"
        width="80px"
        class="icon-ubicacion"
        @click="emitSede('sucre')"
        :fill="esSedeActualSeleccionadaIcon('sucre')"
      >
        <path
          d="M480-480q33 0 56.5-23.5T560-560q0-33-23.5-56.5T480-640q-33 0-56.5 23.5T400-560q0 33 23.5 56.5T480-480Zm0 400Q319-217 239.5-334.5T160-552q0-150 96.5-239T480-880q127 0 223.5 89T800-552q0 100-79.5 217.5T480-80Z"
        />
      </svg>

      <!-- POTOSI -->
      <path
        transform="translate(748,1080)"
        d="m0 0h13l13 4 14 9 10 8 8 11 7 9 8 8 8 7 1 2-3 4h-5l-9-8-8-1-5 5v9l6 11v12l-1 10 2 5 2 1h8l5-1 1 1-1 11-2 8v14l4 12 6 10 11 13 8 7 10 4h7l8-4 3-2 7 1 12 6 3 4 3 10 9 11 6 10v7l-9 8-8 6-12 10-10 6-13 4-14 5-10 6-2 3v8l3 8 1 7-1 17-6 39-4 30v35l3 10 9 11 10 11 7 10v9l-7 18v10l6 24v4l-11 1h-35l-14-8-10-5-28-8-10-1-7 3-6 7-8 12-13 8-11 6-11 11-10 14-5 6-11 9-6 7-12 23-5 7-11 5-46 14-16 3h-11l-3-4-4-15-2-26-4-25-5-15-11-21-7-19-7-28-4-32-5-16-9-16-9-10-14-10-9-6-4-4-1-3v-27l-3-8-5-6-8-7-5-10-6-22-2-11 8-7 8-6 6-7 2-6v-7l-4-11-4-7 1-8 2-6 11 2 24 6 18 7 12 6 10 3h22l14-5 19-9 16-8 15-9 19-12 18-10 13-5 17-4 12-5 19-10 18-12 11-8 1-2v-7l-4-6-9-3-22-3-8-6-8-8-11-15-7-15-7-24-3-14v-21l5-23 3-3 24 6h13l10-5 5-4 14-7z"
        class="hover:fill-[#24FD97]"
        @click="emitSede('potosi')"
        :fill="esSedeActualSeleccionada('potosi')"
      />
      <!-- Ícono SVG -->
      <svg
        xmlns="http://www.w3.org/2000/svg"
        x="5"
        y="1250"
        height="80px"
        viewBox="0 -960 960 960"
        class="icon-ubicacion"
        @click="emitSede('potosi')"
        :fill="esSedeActualSeleccionadaIcon('potosi')"
      >
        <path
          d="M480-480q33 0 56.5-23.5T560-560q0-33-23.5-56.5T480-640q-33 0-56.5 23.5T400-560q0 33 23.5 56.5T480-480Zm0 400Q319-217 239.5-334.5T160-552q0-150 96.5-239T480-880q127 0 223.5 89T800-552q0 100-79.5 217.5T480-80Z"
        />
      </svg>

      <!-- ORURO -->
      <path
        transform="translate(587,1032)"
        d="m0 0 35 2 13 1 5 2 7 8 11 18 6 13v8l-6 28-1 6v20l4 17 8 24 9 17 12 16 8 8 8 5 11 2 17 3 5 4-1 4-5 4-32 20-17 8-9 3-17 4-16 7-20 12-16 10-23 12-16 8-16 6h-17l-13-4-27-12-16-5-18-5 1-5 3-11v-9l-3-10-5-8-1-3-4-2-10-6-19-8-6-4-6-7-12-22-13-24-4-12-1-5v-51l-1-7 9-6 27-14 31-15 17-6 7-2h9l21 6 7 1h13l11-4 11-7 13-12 7-7 14-10z"
        class="hover:fill-[#24FD97]"
        @click="emitSede('oruro')"
        :fill="esSedeActualSeleccionada('oruro')"
      />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        x="550"
        y="1050"
        height="80px"
        viewBox="0 -960 960 960"
        width="80px"
        class="icon-ubicacion"
        @click="emitSede('oruro')"
        :fill="esSedeActualSeleccionadaIcon('oruro')"
      >
        <path
          d="M480-480q33 0 56.5-23.5T560-560q0-33-23.5-56.5T480-640q-33 0-56.5 23.5T400-560q0 33 23.5 56.5T480-480Zm0 400Q319-217 239.5-334.5T160-552q0-150 96.5-239T480-880q127 0 223.5 89T800-552q0 100-79.5 217.5T480-80Z"
        />
      </svg>

      <!-- Tarija -->
      <path
        transform="translate(1008,1462)"
        d="m0 0 4 1 6 12 2 1h24l69-3 105-3h11v8l-5 26-5 19-11 30-13 40-5 13-5 8-2 1h-9l-6-2-3-4-3-15-3-6-5-4-10-3-7-1h-31l-29 4-29 2-12 3-8 5-7 10-15 27-10 17-4 10-2 10v7l3 15v8l-4 8-4 1-4-5-4-12-4-17-8-25-5-18-3-5-12-6-20-6-16-6-11-2h-15l-15 1-3-7-4-18v-12l6-17 1-11 4-11v-15l-7-22-2-12v-7l7 1 8 2 7-6 6 1 10 6 2 1h9l20-9h7l7 4 9 11 8 5 16 3h7l11-3 5-5 2-6 2-12z"
        class="hover:fill-[#24FD97]"
        @click="emitSede('tarija')"
        :fill="esSedeActualSeleccionada('tarija')"
      />
    </svg>
  </div>
</template>
<script setup>
import { defineProps, defineEmits } from "vue";

const props = defineProps(["selectedSede"]);
const emit = defineEmits(["sedeSeleccionada"]);

const emitSede = (sede) => {
  emit("sedeSeleccionada", sede);
};

const esSedeActualSeleccionada = (sede) => {
  return props.selectedSede === sede ? "#24FD97" : "url(#mapGradient)";
};

const esSedeActualSeleccionadaIcon = (sede) => {
  return props.selectedSede === sede ? "#ffffff" : "#24FD97";
};
</script>

<style scoped>
/* Estilo para cambiar el color del ícono al pasar el cursor por el mapa */
path:hover + .icon-ubicacion path {
  fill: #ffffff; /* Verde Potosí */
}
</style>
